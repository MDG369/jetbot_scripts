...
// ./publisher_video_node /home/jetbot/VSLAM-Object-Search/Vocabulary/ORBvoc.txt /home/jetbot/VSLAM-Object-Search/Examples/Stereo/oak-d.yaml /home/jetbot/Documents
...
{  
   ...
   // }
	...
	ros::Publisher pub_camera_pose = nodeHandler.advertise<geometry_msgs::PoseStamped>("camera_pose", 1000);
...
void generateTF(const geometry_msgs::PoseStamped& pose) {
    ...
    transform.setOrigin(tf2::Stamped<tf2::Vector3>(tf2::Vector3(pose.pose.position.x+1200, pose.pose.position.y+500, pose.pose.position.z), ros::Time(), "frame"));
    transform.setRotation(tf2::Stamped<tf2::Quaternion>(tf2::Quaternion(pose.pose.orientation.x,pose.pose.orientation.y, pose.pose.orientation.z, pose.pose.orientation.w),ros::Time(), "frame"));
    geometry_msgs::TransformStamped ts;
    tf2::convert(transform, ts);

    ts.header.stamp = ts.header.stamp;
	...
	ros::Publisher &pub_camera_pose, int frame_id) 
{
	...
	if (pub_all_pts || SLAM.getLoopClosing()->loop_detected || SLAM.getTracker()->loop_detected) {
		...
		camera_pose.pose.position.x = twc.at<float>(0);
		camera_pose.pose.position.y = twc.at<float>(1);
		camera_pose.pose.position.z = twc.at<float>(2);
		...
		pt_array.poses.push_back(camera_pose.pose);
